...
const S = {
  ...
  recipientSuffix: '귀하', // 수신처 형식: '귀하'(회사명 대표자 귀하) 또는 '귀중'(회사명 귀중)
...
function go(id) {
  ...
  apWrap.style.display = 'flex'; /* 모든 서식에서 결재단계 선택 가능 */
  ...
  /* --- 접속 로그 전송 --- */
  if (typeof LOG !== 'undefined') LOG.send(id, def.title, 'view');
}
...
function openModal(id) {
  ...
  /* 수신처 라디오 동기화 */
  document.querySelectorAll('input[name="s-suffix"]').forEach(function(r) {
    r.checked = (r.value === (S.recipientSuffix || '귀하'));
  });
  updateSuffixUI();
  /* 결재단계 버튼 활성화 상태 동기화 */
...
function saveSettings() {
  ...
  // 수신처 형식 저장
  var _sr = document.querySelector('input[name="s-suffix"]:checked');
  if (_sr) S.recipientSuffix = _sr.value;
...
function applyCompanyToForm() {
  ...
  if (S.company) {
    paper.querySelectorAll('.c-company').forEach(el => {
      el.value = S.company; // 항상 덮어쓰기
    });
  }
  if (S.ceo) {
    paper.querySelectorAll('.c-ceo').forEach(el => {
      el.value = S.ceo; // 항상 덮어쓰기
    });
  }
  if (S.addr) {
    paper.querySelectorAll('.c-addr').forEach(el => {
      el.value = S.addr; // 항상 덮어쓰기
    });
  }
  applyRecipientSuffix(paper);
}

/* ── 수신처 형식 적용 ────────────────────────────────────────────
   .c-suffix 클래스 span의 텍스트를 S.recipientSuffix 값으로 갱신
   '귀하' → "회사명 대표자 귀하"  구조에서 suffix 부분
   '귀중' → "회사명 귀중"         구조에서 suffix 부분
   paper 미전달 시 현재 서식에 적용                               */
function applyRecipientSuffix(paper) {
  var p = paper || document.getElementById('a4-paper');
  if (!p) return;
  var suffix = S.recipientSuffix || '귀하';
  p.querySelectorAll('.c-suffix').forEach(function(el) {
    el.textContent = suffix;
  });
  // 귀중 선택 시 c-ceo 행 숨김 (회사명만), 귀하 선택 시 표시
  p.querySelectorAll('.sr-ceo-row').forEach(function(el) {
    el.style.display = (suffix === '귀중') ? 'none' : '';
  });
}
...
function updateSuffixUI() {
  ['1','2'].forEach(function(n) {
    var lbl = document.getElementById('suffix-label-' + n);
    var radio = lbl && lbl.querySelector('input');
    if (!lbl || !radio) return;
    lbl.style.borderColor = radio.checked ? 'var(--navy-dk)' : 'var(--border)';
    lbl.style.background  = radio.checked ? '#e8eef5' : 'var(--light)';
  });
}
...
function printAll() {
  /* 전체 출력 로그 */
  if (typeof LOG !== 'undefined') LOG.send('ALL', '전체 서식 출력', 'print_all');
  /* ── CSS 수집 ──
     단일 파일(인라인 <style>)과 분리 파일(<link>) 모두 대응합니다.
     - <link rel="stylesheet"> 가 있으면 외부 파일 사용
     - 없으면(단일 파일 배포) <style> 태그 인라인 내용 사용    */
  /* CSS 수집 — 외부 링크(구글폰트 등) + 인라인 <style> 모두 포함
     이전에 || 로 했더니 구글폰트 링크만 있으면 인라인 CSS를 건너뛰는 버그 발생
     → + 로 바꿔서 항상 둘 다 포함                                          */
  const linkTags = Array.from(document.querySelectorAll('link[rel="stylesheet"]'))
    .map(l => `<link rel="stylesheet" href="${l.href}">`)
    .join('\n');

  const inlineTags = Array.from(document.querySelectorAll('style'))
    .map(s => `<style>${s.textContent}</style>`)
    .join('\n');

  /* CSS 변수 오버라이드 — applyDesign()이 :root 인라인 스타일로 설정한 값도 포함 */
  const rootVars = document.documentElement.getAttribute('style') || '';
  const rootOverride = rootVars ? `<style>:root { ${rootVars} }</style>` : '';

  const cssLinks = linkTags + '\n' + inlineTags + '\n' + rootOverride;
  ...
  formIds.forEach((id, i) => {
    ...
    /* localStorage에 저장된 편집본 확인 — 있으면 우선 사용 */
    let formContent;
    try {
      const saved = localStorage.getItem('fe_' + id);
      if (saved) {
        /* 저장된 편집본 사용 (결재란 컨테이너 제외한 본문만) */
        const tmp = document.createElement('div');
        tmp.innerHTML = saved;
        /* ap-container(결재란)는 printAll에서 새로 생성하므로 제거 */
        const apCont = tmp.querySelector('#ap-container');
        if (apCont) apCont.remove();
        formContent = tmp.innerHTML;
      } else {
        formContent = def.html();
      }
    } catch(e) {
      formContent = def.html();
    }
...
<style>
  ...
  @media print {
    ...
    .a4-paper {
      ...
      box-shadow: none;
      margin: 0;
      padding: var(--a4-pad-v) var(--a4-pad-h);
    }
...
<!-- ════════════════════════════════════════════
     편집 이력 패널
     ════════════════════════════════════════════ -->
<div id="history-overlay" onclick="FE_HISTORY.closePanel()" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:600"></div>
<div id="history-panel" style="display:none;position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);
  width:560px;max-width:95vw;max-height:80vh;background:#fff;border-radius:8px;
  box-shadow:0 20px 60px rgba(0,0,0,.3);z-index:601;display:none;flex-direction:column;overflow:hidden">
  <div style="background:#0d2137;color:#fff;padding:14px 18px;display:flex;align-items:center;justify-content:space-between;flex-shrink:0">
    <div>
      <div style="font-size:14px;font-weight:500">📋 편집 이력</div>
      <div id="history-panel-sub" style="font-size:11px;color:rgba(255,255,255,.5);margin-top:2px"></div>
    </div>
    <button onclick="FE_HISTORY.closePanel()" style="background:transparent;border:none;color:rgba(255,255,255,.6);font-size:18px;cursor:pointer">✕</button>
  </div>
  <div style="padding:10px 16px;background:#f5f5f5;border-bottom:1px solid #eee;font-size:11.5px;color:#666;flex-shrink:0">
    저장된 버전을 클릭하면 미리보기 — <strong>복원</strong> 버튼으로 해당 버전으로 되돌립니다.
    최대 <strong>10개</strong> 버전 보관 (초과 시 가장 오래된 버전 자동 삭제)
  </div>
  <div id="history-list" style="flex:1;overflow-y:auto;padding:8px 0"></div>
  <div style="padding:12px 16px;background:#fafafa;border-top:1px solid #eee;display:flex;justify-content:space-between;align-items:center;flex-shrink:0">
    <button onclick="FE_HISTORY.clearAll()" style="background:transparent;border:1px solid #ddd;color:#999;padding:5px 10px;border-radius:4px;font-size:12px;cursor:pointer;font-family:sans-serif">🗑 전체 삭제</button>
    <button onclick="FE_HISTORY.closePanel()" style="background:#0d2137;color:#fff;border:none;padding:6px 16px;border-radius:4px;font-size:12.5px;cursor:pointer;font-family:sans-serif">닫기</button>
  </div>
</div>

<!-- docx.js + FileSaver — Word 출력용 -->
...
<script>
(function() {
  var company = ${JSON.stringify(S.company || '')};
  var ceo     = ${JSON.stringify(S.ceo     || '')};
  var addr    = ${JSON.stringify(S.addr    || '')};
  var suffix  = ${JSON.stringify(S.recipientSuffix || '귀하')};
  if (company) document.querySelectorAll('.c-company').forEach(function(el) { el.value = company; });
  if (ceo)     document.querySelectorAll('.c-ceo').forEach(function(el) { el.value = ceo; });
  if (addr)    document.querySelectorAll('.c-addr').forEach(function(el) { el.value = addr; });
  document.querySelectorAll('.c-suffix').forEach(function(el) { el.textContent = suffix; });
  document.querySelectorAll('.sr-ceo-row').forEach(function(el) {
    el.style.display = (suffix === '귀중') ? 'none' : '';
  });
})();
</script>
...
(function init() {
  ...
  /* 새로고침 시 항상 홈으로 — 저장된 currentForm 무시 */
  delete S.currentForm;
  document.getElementById('screen-home').classList.add('active');
})();
...
/* ─────────────────────────────────────────────────────────────
   ...
   Chrome 인쇄 엔진은 flex 아이템의 margin-top:auto를 무시하는 버그가 있음.
   beforeprint 시 margin-top:0 으로 자연 높이를 측정한 뒤 spacer 삽입.
   afterprint 시 spacer 제거.
   ───────────────────────────────────────────────────────────── */
window.addEventListener('beforeprint', function() {
  ...
  var padTop  = parseFloat(paperStyle.paddingTop)    || 0;
  var padBot  = parseFloat(paperStyle.paddingBottom) || 0;

  /* margin-top:auto 를 잠시 0으로 → 자연 위치 측정 */
  var prevMargin = signArea.style.marginTop;
  signArea.style.marginTop = '0';
  var _reflow = signArea.offsetTop; /* 강제 reflow */

  var contentAreaH = paper.clientHeight - padTop - padBot;
  var naturalBeforeH = signArea.offsetTop - padTop;  /* auto 마진 제외한 위치 */
  var signH = signArea.offsetHeight;

  /* 원래 값 복원 */
  signArea.style.marginTop = prevMargin;

  var spacerH = contentAreaH - naturalBeforeH - signH;

  if (spacerH > 5) {
    var spacer = document.createElement('div');
    spacer.className = 'print-sign-spacer';
    spacer.style.cssText = 'height:' + spacerH + 'px;flex-shrink:0;flex-grow:0';
    signArea.parentNode.insertBefore(spacer, signArea);
  }
});

window.addEventListener('afterprint', function() {
  document.querySelectorAll('.print-sign-spacer').forEach(function(el) { el.remove(); });
});
...
window.addEventListener('beforeunload', () => {
  try {
    ...
    delete toSave.currentForm;
    localStorage.setItem('laborPortalSettings', JSON.stringify(toSave));
  } catch(e) {
    /* 저장 실패 시 무시 */
  }
});
