...
const DEFAULT_DESIGN = {
  ...
  labelFont:   12,          /* ② 분류명 — 테이블 라벨·헤더 셀 (pt) */
  contentFont: 12,          /* ③ 내용   — 입력값·본문 (pt) */
  etcFont:     12,          /* ④ 기타   — 블록박스·부제목 (pt) */
...
function applyDesign(d) {
  ...
  /* 크기 (단위 포함하여 설정) */
  r.setProperty('--form-font',    d.formFont    + 'pt');  /* 하위호환 */
  r.setProperty('--title-font',   d.titleFont   + 'pt');  /* ① 제목 */
  r.setProperty('--label-font',   d.labelFont   + 'pt');  /* ② 분류명 */
  r.setProperty('--content-font', d.contentFont + 'pt');  /* ③ 내용 */
  r.setProperty('--etc-font',     d.etcFont     + 'pt');  /* ④ 기타 */
...
function syncEditorUI() {
  ...
  setSlider('ed-labelFont',   DESIGN.labelFont,   'ed-labelFont-v',   'pt');
  setSlider('ed-contentFont', DESIGN.contentFont, 'ed-contentFont-v', 'pt');
  setSlider('ed-etcFont',     DESIGN.etcFont,     'ed-etcFont-v',     'pt');
...
function edChange(key, value, type) {
  ...
  /* 슬라이더인 경우 옆 숫자 표시 갱신 */
  const unitMap = {
    titleFont:   'pt', labelFont:   'pt', contentFont: 'pt', etcFont: 'pt',
    formFont:    'pt', /* 하위호환 */
    cellPadV:    'pt', signW: 'pt', docBorder: 'pt',
    a4PadV: 'mm', a4PadH: 'mm'
  };
  if (unitMap[key]) {
...
function exportCSS() {
  ...
  /* ── 서식 크기 ── */
  --title-font:   ${DESIGN.titleFont}pt;    /* ① 제목 */
  --label-font:   ${DESIGN.labelFont}pt;    /* ② 분류명 */
  --content-font: ${DESIGN.contentFont}pt;  /* ③ 내용 */
  --etc-font:     ${DESIGN.etcFont}pt;      /* ④ 기타 */
