...
var FE = (function() {
  ...
  /* 편집 대상 — 단일 CSS 선택자 문자열
     여기에 쉼표로 구분하여 선택자를 추가하면 편집 범위가 늘어납니다.
     ※ .ft td / .ft th 로 서식 테이블 전체 셀을 한 번에 커버합니다.  */
  var EDITABLE_SEL = [
    '.ft td',            /* 서식 테이블 모든 데이터 셀            */
    '.ft th',            /* 서식 테이블 헤더 셀                   */
    '.doc-title',        /* 서식 제목                             */
    '.doc-law',          /* 법령 근거                             */
    '.body-text',        /* 본문 텍스트 (input 혼재 허용)         */
    '.block-box',        /* 박스 텍스트 (input 혼재 허용)         */
    '.sec-label',        /* 섹션 레이블                           */
    '.ap-box-name',      /* 결재란 직급                           */
    '.ap-label',         /* 결재란 "결재" 텍스트                  */
  ...
  /* ── applyEditable(root) : root 안의 편집 대상에 contenteditable 일괄 적용 ──
     enter() 및 행 추가 시 공통 호출                                         */
  function applyEditable(root) {
    root.querySelectorAll(EDITABLE_SEL).forEach(function(el) {
  ...
  function hasInput(el) {
    /* .body-text / .block-box 는 내부에 input이 있어도 편집 허용
       (회사명 input은 그대로 두고 주변 텍스트만 contenteditable로 감쌈) */
    if (el.classList &&
        (el.classList.contains('body-text') || el.classList.contains('block-box'))) {
      return ['INPUT','SELECT','TEXTAREA'].indexOf(el.tagName) >= 0;
    }
    /* .ft .val 도 내부에 input이 있으면 편집 불가 (input 자체로 입력하게) */
  ...
  function enter() {
    ...
    /* ── 편집 가능 셀 시각적 표시 스타일 주입 (최초 1회) ── */
    if (!document.getElementById('fe-edit-style')) {
      var st = document.createElement('style');
      st.id = 'fe-edit-style';
      st.textContent = [
        /* 편집 가능 요소 — 호버 시 연한 파란 테두리 */
        '[data-fe-editable]:hover {',
        '  outline: 2px solid #4a90d9 !important;',
        '  outline-offset: -1px;',
        '  cursor: text;',
        '}',
        /* 포커스 시 */
        '[data-fe-editable]:focus {',
